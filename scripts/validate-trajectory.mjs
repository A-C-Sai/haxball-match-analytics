 ...
 * A failure here means the physics model is wrong — do NOT loosen the
 * tolerance to make it pass. See README-session-event-capture.md for why.
 */
...
for (let trial = 0; trial < TRIALS; trial++) {
  const sx = (rand() - 0.5) * 700, sy = (rand() - 0.5) * 300;
  ...
  for (let i = STADIUM_DISCS; i < ps.discs.length; i++) {
    ps.discs[i].pos.x = 4000; ps.discs[i].pos.y = 4000;
  ...
  for (let t = 0; t < TICKS; t++) {
    ...
    for (let i = STADIUM_DISCS; i < live.length; i++) {
      live[i].pos.x = 4000; live[i].pos.y = 4000; live[i].speed.x = 0; live[i].speed.y = 0;
    }
...
console.log(`\nsamples ${errs.length}, ticks containing a bounce ${bounceTicks}, trials cut by an engine reset ${cutByEngine}/${TRIALS}`);
...
console.log(`\n${pass ? 'PASS' : 'FAIL'} — predictor ${pass ? 'reproduces' : 'does NOT reproduce'} engine arithmetic`);

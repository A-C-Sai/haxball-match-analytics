export default function(API, params){
  ...
  function _doRender() {
    ...
    // Exposed publicly (read-only, by convention) so external overlay code
    // can replicate this exact camera transform without duplicating the
    // origin-follow/zoom-clamp logic above: screenX = (mapX - cameraOrigin.x) * cameraScale + canvasWidth/2,
    // screenY = (mapY - cameraOrigin.y) * cameraScale + canvasHeight/2 — see stage2.x/y just above.
    thisRenderer.cameraOrigin = { x: origin.x, y: origin.y };
    thisRenderer.cameraScale = scale;
